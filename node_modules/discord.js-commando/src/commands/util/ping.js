...
module.exports = class PingCommand extends Command {
	...
	async run(msg, {inp}) {
		if(inp>3){
			msg.reply("Can't ping more than 3 times");
		}
		else if(inp==1){
			if(!msg.editable) {
			...
			for(var i=0; i<inp; i++)
			{
				...
				pingMsg.edit(oneLine`
					${msg.channel.type !== 'dm' ? `${msg.author},` : ''}
					Pong! The message round-trip took ${pingMsg.createdTimestamp - msg.createdTimestamp - tmp1}ms.
					${this.client.ping ? `The heartbeat ping is ${Math.round(this.client.ping)}ms...` : ''}
				`);
				sum += pingMsg.createdTimestamp - msg.createdTimestamp - tmp1;
				tmp1 = pingMsg.createdTimestamp - msg.createdTimestamp;
			}
			msg.channel.send("Average one way ping is " + Math.round(sum/inp)/2);
		}
	}
};
